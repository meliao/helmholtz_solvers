...
class Test_bicgstab_batch:
    ...
    def test_2(self) -> None:
        ...
        print("First element of resid_norm_lst", out_info["resid_norm_lst"][0].shape)
        assert out_info["resid_norm_lst"][0].shape == (K, m), out_info[
            "resid_norm_lst"
        ][0].shape
    ...
    def test_5(self) -> None:
        """Tests convergence of a large, ill-conditioned Real system with a random A matrix. A is
        designed to be clearly PD, with rapidly-decaying eigenvalues.
        Also, uses a real vector for B"""
        torch.manual_seed(0)
        np.random.seed(0)
        ...
        R = special_ortho_group.rvs(n)
        D = np.diag(np.exp(-np.linspace(1, 10, n)))
        A = R @ D @ R.T
        A_inv = R @ np.diag(1 / np.diag(D)) @ R.T
        A = torch.from_numpy(A)
        A_inv = torch.from_numpy(A_inv)
        ...
        # This is too hard to satisfy with the poorly-conditioned A matrix
        # assert np.allclose(AA_inv.numpy(), np.eye(n), atol=1e-5, rtol=1e-5)
        ...
        print("Exited after ", out_info["niter"], " iterations")
        print("Residual norms: ", np.array(out_info["resid_norm_lst"]).flatten())
    ...
    @pytest.mark.skip(reason="Not using preconditioning anymore")
    def test_7(self) -> None:
        """Tests convergence of a large, ill-conditioned diagonal matrix with rapidly-decaying eigenvalues.
        ...
        R = special_ortho_group.rvs(n)
        diag_a = np.exp(-np.linspace(1, 15, n))
        D = np.diag(diag_a)

        A = D
        A_inv = np.diag(1 / np.diag(D))
        A = torch.from_numpy(A)
        A_inv = torch.from_numpy(A_inv)
        ...
        diag_a = torch.from_numpy(diag_a)

        def _K(x: torch.Tensor) -> torch.Tensor:
            print("diag_a", diag_a.shape)
            print("x", x.shape)
            return x / diag_a.unsqueeze(0).unsqueeze(-1)
        ...
        # print(out, out.shape)
        # print(diag_a, diag_a.shape)
        # out = out / diag_a.unsqueeze(0).unsqueeze(-1)
        ...
        assert out.shape == (K, n, m)
        assert np.allclose(
            out_resid.numpy(), np.zeros_like(out_resid.numpy()), atol=1e-05, rtol=1e-05
        )
